...
**Boundary vs `audit`:** `audit` *measures and reports* perf issues (scoring P0–P3, objective facts); `optimize` *fixes* them (implements the solutions). When uncertain: run `audit` first to get a scored diagnosis, then `optimize` to address the findings.
...
1. `<project>/.claude/lessons/optimize.md`  (preferred when inside a project)
2. `{AGENCY_STATE}/lessons/optimize.md`           (fallback when there is no project context)
...
- One physical line, exact format:
  `- YYYY-MM-DD <imperative fix or invariant> [ctx: optimize/<project-or-->]`
...
`python3 "${SIL_KERNEL:-$HOME/.claude/lib/self-improving-loop}"/sil/cli.py lessons-append \
  --file "<resolved-path>" --date "YYYY-MM-DD" --window "run" --note "<imperative rule>" --tweak "optimize/<project>"`

## Recommended next step

Next: `/audit` — confirm no a11y/anti-pattern regressions were introduced during optimization.