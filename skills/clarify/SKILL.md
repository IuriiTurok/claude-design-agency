...
Read `${CLAUDE_PLUGIN_ROOT}/skills/impeccable/context-protocol.md` and follow the Context Gathering Protocol. If no design context exists, run `/impeccable teach` first. Additionally gather: audience technical level and users' mental state in context.
...
> **UX writing canon:** for deeper tone, voice, and writing principles, see `${CLAUDE_PLUGIN_ROOT}/skills/impeccable/reference/ux-writing.md` — the canonical source for copy standards shared across design micro-skills.

## Self-learning

This skill learns across runs via a lessons file. Resolve its path once, first hit wins:
1. `<project>/.claude/lessons/clarify.md`  (preferred when inside a project)
2. `{AGENCY_STATE}/lessons/clarify.md`  (fallback when there is no project context)
...
- One physical line, exact format:
  `- YYYY-MM-DD <imperative fix or invariant> [ctx: clarify/<project-or-->]`
...
`python3 "${SIL_KERNEL:-$HOME/.claude/lib/self-improving-loop}"/sil/cli.py lessons-append \
  --file "<resolved-path>" --date "YYYY-MM-DD" --window "run" --note "<imperative rule>" --tweak "clarify/<project>"`

## Recommended next step

Next: `/critique` — re-evaluate the now-clearer UX holistically.