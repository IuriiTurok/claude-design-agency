...
description: Apply Emil Kowalski's design-engineering philosophy to UI polish, component
  design, and animation decisions — the invisible details that make software feel
  hand-crafted. Use when the user wants senior design-engineering review of a component,
  asks whether an interaction "feels right", debates an animation/easing/timing choice,
  or wants taste-level guidance on the small details (hover, focus, spacing, motion) that
  separate good UI from great UI.
---
...
1. `<project>/.claude/lessons/emil-design-eng.md`  (preferred when inside a project)
2. `{AGENCY_STATE}/lessons/emil-design-eng.md`  (fallback when there is no project context)
...
- One physical line, exact format:
  `- YYYY-MM-DD <imperative fix or invariant> [ctx: emil-design-eng/<project-or-->]`
...
`python3 ~/.claude/lib/self-improving-loop/sil/cli.py lessons-append \
  --file "<resolved-path>" --date "YYYY-MM-DD" --window "run" --note "<imperative rule>" --tweak "emil-design-eng/<project>"`

## Recommended next step

Next: `/polish` — fold the component-level taste notes from this review into the final polish pass.
