...
class DesignSystemGenerator:
    ...
    def generate(self, query: str, project_name: str = None) -> dict:
        ...
        return {
            ...
            # Carried for the sil persisted-projects archive: the reasoning rule's
            # id + its generated field defaults, keyed exactly to the loop's fields.
            "rule_id": reasoning.get("rule_id"),
            "reasoning_fields": {
                "pattern": reasoning.get("pattern", ""),
                "style": " + ".join(reasoning.get("style_priority", [])),
                "color_mood": reasoning.get("color_mood", ""),
                "typography_mood": reasoning.get("typography_mood", ""),
                "key_effects": reasoning.get("key_effects", ""),
            },
        }
...
def generate_design_system(
    ...
    if persist:
        persist_design_system(design_system, page, output_dir, query, overrides)
...
def persist_design_system(
    ...
    # Feed the sil persisted-projects archive (the realized override-rate corpus).
    _archive_persisted_project(design_system, overrides)
...
def _archive_persisted_project(
    design_system: dict, overrides: dict | None = None
) -> None:
    """Append one {ts, project, rule_id, fields, overrides} record to
    ~/.claude/cache/ui-ux-pro-max/persisted-projects.jsonl — the realized corpus
    ui-ux-pro-max_loop.py reads. `fields` are the reasoning rule's generated
    defaults (carried on design_system by generate()); `overrides` are the
    field-level edits the persisting caller applied on the user's behalf (empty
    when persisted as-generated). One record per persist call (no double-count).
    Fail-open: any error is swallowed so persistence never breaks on archival."""
    try:
        rule_id = design_system.get("rule_id")
        fields = design_system.get("reasoning_fields") or {}
        if rule_id is None or not fields:
            return  # nothing the loop can key on
        cache = Path.home() / ".claude" / "cache" / "ui-ux-pro-max"
        ...
        rec = {
            ...
            "rule_id": str(rule_id),
            "fields": fields,
            "overrides": {k: v for k, v in (overrides or {}).items() if k in fields},
        }
        with open(cache / "persisted-projects.jsonl", "a") as f:
            f.write(json.dumps(rec) + "\n")
    except Exception:
        pass  # fail-open: archival must never break persistence
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--persist",
        action="store_true",
        help="persist the design system AND append a record to the sil persisted-projects archive",
    )
    parser.add_argument(
        "--override",
        action="append",
        default=[],
        metavar="FIELD=VALUE",
        help="a user override on the generated default; repeatable "
        "(e.g. --override style='Flat Design'). Recorded in the sil archive.",
    )
    parser.add_argument(
        "--page",
        default=None,
        help="optional page name for a page-specific override file (with --persist)",
    )
