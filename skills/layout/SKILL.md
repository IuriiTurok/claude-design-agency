...
**Boundary — layout vs siblings:** `layout` is the **structure-first stage** of the `/refine` pipeline: it fixes spatial composition (spacing system, grid, rhythm, hierarchy) before type or color are tuned. If the problem is specifically typography within a settled layout, go to `/typeset`. If the problem is color/palette only, go to `/colorize`. If the layout is correct but too complex/cluttered, go to `/distill` (removes elements). If the layout needs to hold across breakpoints or new device targets, follow up with `/adapt`.

**In the `/refine` chain:** `audit → layout → typeset → colorize → harden → polish`. Run `layout` after `audit` surfaces structure issues; run `/typeset` once the spatial rhythm is set.
...
1. `<project>/.claude/lessons/layout.md`  (preferred when inside a project)
2. `{AGENCY_STATE}/lessons/layout.md`           (fallback when there is no project context)
...
- One physical line, exact format:
  `- YYYY-MM-DD <imperative fix or invariant> [ctx: layout/<project-or-->]`
...
`python3 "${SIL_KERNEL:-$HOME/.claude/lib/self-improving-loop}"/sil/cli.py lessons-append \
  --file "<resolved-path>" --date "YYYY-MM-DD" --window "run" --note "<imperative rule>" --tweak "layout/<project>"`

---

## Recommended next step

Next: `/typeset` — structure and spatial rhythm are now settled; tune the type scale, weights, and leading within the new layout to complete the composition.