...
**Boundary — distill vs quieter:** Use `distill` when the problem is *too much stuff* — clutter, redundant elements, feature creep, no progressive disclosure. Distill **removes elements** (changes the content/structure). Use `quieter` when the problem is *too much intensity* — the right elements are present but they shout. Quieter **turns down** what stays (changes the styling, not the inventory). A design can be both loud and cluttered → run `distill` first (remove), then `quieter` (tone down).
...
1. `<project>/.claude/lessons/distill.md`  (preferred when inside a project)
2. `{AGENCY_STATE}/lessons/distill.md`           (fallback when there is no project context)
...
- One physical line, exact format:
  `- YYYY-MM-DD <imperative fix or invariant> [ctx: distill/<project-or-->]`
...
`python3 "${SIL_KERNEL:-$HOME/.claude/lib/self-improving-loop}"/sil/cli.py lessons-append \
  --file "<resolved-path>" --date "YYYY-MM-DD" --window "run" --note "<imperative rule>" --tweak "distill/<project>"`

## Recommended next step

Next: `/polish` — polish what remains after clutter is removed.