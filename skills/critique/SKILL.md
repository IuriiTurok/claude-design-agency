...
**Boundary — audit vs critique:** Use `audit` for *measurable, code-level* facts (WCAG contrast ratios, missing ARIA, responsive breakpoints, P0–P3 scored anti-patterns). Use `critique` for a *subjective UX/design-director* read (visual hierarchy, information architecture, emotional resonance, cognitive load, AI-slop tells). `audit ⊂ critique`: for a thorough review, `critique` should call `audit` for the objective dimensions and fold those scores into its report, then add the subjective assessment on top.

## STEPS

### Step 1: Preparation

Read `${CLAUDE_PLUGIN_ROOT}/skills/impeccable/context-protocol.md` and follow the Context Gathering Protocol. If no design context exists, run `/impeccable teach` first. Additionally gather: what the interface is trying to accomplish.
...
- **Motion wiring**: if Nielsen Heuristic #1 (Visibility of System Status) scored ≤2 — OR issues involve missing loading states, unacknowledged actions, or jarring transitions — include **`/animate`** ("add feedback, loading states, and state transitions, clamped to the project motion budget") in the recommendations. A low #1 score is the signal that motion/feedback is missing; do not let it go unaddressed.
...
1. `<project>/.claude/lessons/critique.md`  (preferred when inside a project)
2. `{AGENCY_STATE}/lessons/critique.md`           (fallback when there is no project context)
...
- One physical line, exact format:
  `- YYYY-MM-DD <imperative fix or invariant> [ctx: critique/<project-or-->]`
...
`python3 "${SIL_KERNEL:-$HOME/.claude/lib/self-improving-loop}"/sil/cli.py lessons-append \
  --file "<resolved-path>" --date "YYYY-MM-DD" --window "run" --note "<imperative rule>" --tweak "critique/<project>"`

## Recommended next step

Next: `/refine` — subjective findings in hand → run the mechanical fix pipeline (audit→layout→typeset→colorize→harden→polish); re-run `/critique` after to confirm the score improved.