...
**Pipeline stage:** This skill is the `harden` stage of the `/refine` orchestrator (`audit → layout → typeset → colorize → **harden** → polish`). It can also run standalone.

## Mandatory Preparation

Before hardening, load shared design context:

1. Read `${CLAUDE_PLUGIN_ROOT}/skills/impeccable/context-protocol.md` (or run `/impeccable teach`) to load the current project's design language, token palette, and component conventions. This prevents hardening states from introducing inconsistent styling.
2. If inside a project, also check for `.claude/lessons/harden.md` (see Self-learning below).
...
1. `<project>/.claude/lessons/harden.md`  (preferred when inside a project)
2. `{AGENCY_STATE}/lessons/harden.md`           (fallback when there is no project context)
...
- One physical line, exact format:
  `- YYYY-MM-DD <imperative fix or invariant> [ctx: harden/<project-or-->]`
...
`python3 "${SIL_KERNEL:-$HOME/.claude/lib/self-improving-loop}"/sil/cli.py lessons-append \
  --file "<resolved-path>" --date "YYYY-MM-DD" --window "run" --note "<imperative rule>" --tweak "harden/<project>"`

## Recommended next step

Next: `/polish` — final pass over the new states.