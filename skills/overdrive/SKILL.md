...
1. `<project>/.claude/lessons/overdrive.md`  (preferred when inside a project)
2. `{AGENCY_STATE}/lessons/overdrive.md`           (fallback when there is no project context)
...
- One physical line, exact format:
  `- YYYY-MM-DD <imperative fix or invariant> [ctx: overdrive/<project-or-->]`
...
`python3 "${SIL_KERNEL:-$HOME/.claude/lib/self-improving-loop}"/sil/cli.py lessons-append \
  --file "<resolved-path>" --date "YYYY-MM-DD" --window "run" --note "<imperative rule>" --tweak "overdrive/<project>"`

## Recommended next step

Next: `/optimize` — make the ambitious build actually fast and robust.